...
import { LogOut, Users, Settings, BarChart3, Shield, Save, Trash2, AlertTriangle } from "lucide-react";
...
const SecureAdminDashboard = () => {
  const { adminUser, signOut, isAuthenticated, loading: authLoading } = useSecureAdminAuth();
  ...
  const updateSetting = async (settingId: string, newValue: string) => {
    try {
      ...
      if (!newValue.trim()) {
        toast({
          title: "Erro",
          description: "O valor da configuração não pode estar vazio",
          variant: "destructive"
        });
        return;
      }
      ...
      setSettings(prev => 
        prev.map(setting => 
          setting.id === settingId 
            ? { ...setting, setting_value: newValue.trim(), updated_at: new Date().toISOString() }
            : setting
        )
      );
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>Resumo do Sistema Seguro</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-green-600 font-semibold">Online e Seguro</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Autenticação:</span>
                    <span className="text-green-600 font-semibold">✓ Protegida</span>
                  </div>
                </div>
              </CardContent>
            </Card>
                    ...
                    <div className="flex space-x-2">
                      {setting.setting_type === 'textarea' ? (
                        ...
                        <Input
                          ...
                          className="flex-1"
                          maxLength={setting.setting_type === 'email' ? 254 : 255}
                        />
                      )}
                      <Button 
